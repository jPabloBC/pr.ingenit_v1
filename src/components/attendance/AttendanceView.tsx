...
export function AttendanceView({ renderImportAction }: AttendancePageProps = {}) {
  ...
  const canEditSelectedDailyAttendance = canEditDailyAttendance || (currentRole === 'user' && dailyDate === defaultTo)
  ...
  const attendanceTableHeaderSx = {
    bgcolor: colors.blue4,
    color: colors.white,
    borderBottom: `1px solid ${colors.blue8}`,
    '& .MuiTableSortLabel-root': {
      color: `${colors.white} !important`,
      fontWeight: 800,
    },
    '& .MuiTableSortLabel-root:hover': {
      color: `${colors.blue13} !important`,
    },
  ...
  const getAttendanceRowBg = (index: number): string => (index % 2 === 0 ? colors.white : '#f6f9fc')

  const getStickyCellSx = (key: string, isHeader = false, rowBg: string = colors.white) => ({
    ...
    ...(isPinnedHistoryColumn(key)
      ? {
          ...
          zIndex: isHeader ? 6 : 4,
          backgroundColor: isHeader ? colors.blue4 : rowBg,
  ...
  const historyPinButtonSx = (active: boolean) => ({
    ...
    p: 0,
    mr: 0.75,
    color: active ? colors.white : colors.blue13,
  ...
  const handleDailyStatusChange = (collaboratorId: string, nextStatus: string) => {
    if (!canEditSelectedDailyAttendance) return
          ...
          <Paper variant="outlined" sx={{ p: 0, width: '100%', maxWidth: '100%', overflowX: 'hidden', borderColor: colors.managementBorder, borderRadius: 1.5 }}>
            {loading ? (
              ...
              <>
                ...
                <TableContainer sx={{ maxHeight: '72vh', borderTop: `1px solid ${colors.blue4}` }}>
                  <Table
                    stickyHeader
                    size="small"
                    sx={{
                      ...
                      '& .MuiTableBody-root .MuiTableRow-root:hover': {
                        bgcolor: `${colors.blue15} !important`,
                      },
              ...
              <>
                ...
                <TableContainer
                  sx={{
                    ...
                    overflowX: 'auto',
                    overflowY: 'auto',
                    borderTop: `1px solid ${colors.blue4}`,
                  }}
                >
                  <Table
                    stickyHeader
                  ...
                  >
                    <TableHead>
                      <TableRow sx={{ backgroundColor: colors.blue4 }}>
                        <TableCell
                          align="center"
                          sx={{
                            position: 'sticky',
                        ...
                        {historyDateColumns.map((date) => (
                          <TableCell
                            key={`col-${date}`}
                            align="center"
                            sx={{ minWidth: 120, maxWidth: 120, fontWeight: 800, color: colors.white }}
                          >
                    ...
                    <TableBody>
                      {filteredHistoryRows.map((row, rowIndex) => {
                        const rowBg = getAttendanceRowBg(rowIndex)
                        return (
                        <TableRow key={row.collaborator_id} hover sx={{ bgcolor: rowBg }}>
                          <TableCell
                            align="center"
                            sx={{
                              position: 'sticky',
                          ...
                          <TableCell sx={getStickyCellSx('document', false, rowBg)}>
                            <Tooltip
                              arrow
                              placement="right"
                              disableInteractive={false}
                          ...
                          <TableCell sx={getStickyCellSx('name', false, rowBg)}>
                            <Typography variant="body2" noWrap sx={{ fontSize: '0.8rem', fontWeight: 600 }}>
                              {String(formatFullName(row.collaborator)).toUpperCase()}
                            </Typography>
                          </TableCell>
                          <TableCell sx={getStickyCellSx('position', false, rowBg)}>
                            <Typography variant="body2" noWrap sx={{ fontSize: '0.8rem' }}>
                              {String(row.collaborator?.position || '').toUpperCase()}
                            </Typography>
                          </TableCell>
                          <TableCell align="center" sx={getStickyCellSx('worker_type', false, rowBg)}>
                            <Typography variant="body2" noWrap sx={{ fontSize: '0.8rem' }}>
                              {String(row.collaborator?.worker_type || '').toUpperCase()}
                            </Typography>
                          </TableCell>
                          <TableCell sx={getStickyCellSx('specialty', false, rowBg)}>
                            <Typography variant="body2" noWrap sx={{ fontSize: '0.8rem' }}>
                              {String(row.collaborator?.specialty || '').toUpperCase()}
                            </Typography>
                          </TableCell>
                          <TableCell align="center" sx={getStickyCellSx('is_active', false, rowBg)}>
