import { escapeHtml, isSafeHttpUrl } from '@/lib/sanitizeHtml'
...
export function resetPasswordEmail({ name, resetUrl }: { name?: string; resetUrl: string }) {
  ...
  const safeName = name ? `Hola ${escapeHtml(name)},` : 'Hola,'
  const safeResetUrl = isSafeHttpUrl(resetUrl) ? escapeHtml(resetUrl) : ''
            ...
            <table role="presentation" cellpadding="0" cellspacing="0" width="600" style="background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 6px rgba(16,24,40,.08);">
              <tr>
                <td style="padding:24px 32px">
                  <h1 style="margin:0 0 8px;font-size:20px;color:#0f172a">${safeAppName}</h1>
                  ...
                  <p style="text-align:center;margin:24px 0">
                    <a href="${safeResetUrl}" style="background:#2563eb;color:#fff;padding:12px 20px;border-radius:6px;text-decoration:none;display:inline-block">Restablecer contraseña</a>
                  </p>
                  <p style="margin:0 0 12px;color:#94a3b8;font-size:13px">Si el botón no funciona, pega este enlace en tu navegador:</p>
                  <p style="word-break:break-all;color:#0f172a;font-size:13px">${safeResetUrl}</p>
