...
import { AttachFile, Close, ContentCopyOutlined, EmailOutlined, ManageAccountsOutlined, PictureAsPdfOutlined, SendOutlined, ShareOutlined, WhatsApp } from '@mui/icons-material'
import { CircleArrowLeft, CircleArrowRight, MailX, MessageCircleOff } from 'lucide-react'
import UserHeader from '@/components/layout/UserHeader'
import { colors } from '@/theme/theme'

type Collaborator = { id: string; name: string; position: string; specialty: string; phone: string; email: string }
...
export default function CommunicationsPage() {
  ...
  const matchesCollaboratorSearch = (person: Collaborator) => !searchTerm || [person.name, person.position, person.email, person.phone].some((value) => normalizeSearch(value).includes(searchTerm))
  const matchingPositions = useMemo(() => positions.filter((position) => !searchTerm || normalizeSearch(position).includes(searchTerm) || collaborators.some((person) => person.position === position && matchesCollaboratorSearch(person))), [positions, collaborators, searchTerm])
  const matchingCollaborators = useMemo(() => filteredCollaborators.filter(matchesCollaboratorSearch), [filteredCollaborators, searchTerm])
  ...
  const copyContact = async (value: string, label: string) => {
    try {
      ...
      setToast({ type: 'success', text: `${label} copiado.` })
    } catch {
      setToast({ type: 'error', text: `No fue posible copiar el ${label.toLocaleLowerCase('es-CL')}.` })
    }
  }
  ...
  const send = async () => {
    if (!selectedPositions.length) return setToast({ type: 'info', text: 'Selecciona al menos un cargo.' })
    if (!selectedIds.length) return setToast({ type: 'info', text: 'Selecciona al menos un destinatario.' })
    if (!title.trim() || !message.trim()) return setToast({ type: 'info', text: 'Completa el asunto y el mensaje.' })
    if (!channels.length) return setToast({ type: 'info', text: 'Selecciona al menos un canal.' })
    setSending(true)
    try {
      ...
      setToast({ type: 'success', text: `Campaña creada para ${result.recipients} destinatarios. Correos enviados: ${result.email_sent}.${whatsappSummary}` })
      setTitle('')
      setMessage('')
      setAttachment(null)
      setSelectedIds([])
      await load()
      if (channels.includes('whatsapp') && !result.whatsapp_automated) await openCampaign(result.campaign_id)
    } catch (error) {
      setToast({ type: 'error', text: error instanceof Error ? error.message : String(error) })
    } finally {
      setSending(false)
  ...
  const sharePdf = async (delivery: Delivery) => {
    if (!delivery.attachment_url) return setToast({ type: 'info', text: 'Esta campaña no tiene PDF adjunto.' })
    try {
      ...
      setToast({ type: 'info', text: 'El navegador descargó el PDF. Adjuntalo manualmente en WhatsApp.' })
    } catch (error) {
      setToast({ type: 'error', text: error instanceof Error ? error.message : String(error) })
    }
  }
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: colors.managementWhiteSoft }}>
      ...
      <Box sx={{ p: { xs: 1.5, md: 2.5 }, maxWidth: 1480, mx: 'auto' }}>
        <Stack spacing={2}>
          {toast && <Alert severity={toast.type} onClose={() => setToast(null)}>{toast.text}</Alert>}
          {loading ? <Box sx={{ py: 8, textAlign: 'center' }}><CircularProgress /></Box> : <>
            <Paper variant="outlined" sx={{ p: { xs: 1.5, md: 2 }, borderColor: colors.managementBorder }}>
              <Stack spacing={1.5}>
                <Box sx={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 1.5, flexWrap: 'wrap' }}>
                  ...
                  <TextField value={positionSearch} onChange={(event) => setPositionSearch(event.target.value)} size="small" label="Buscar destinatario" placeholder="Cargo, nombre, correo o teléfono" InputLabelProps={{ sx: { fontSize: 14 } }} InputProps={{ endAdornment: positionSearch ? <Tooltip title="Limpiar búsqueda"><IconButton size="small" aria-label="Limpiar búsqueda" onClick={() => setPositionSearch('')} edge="end" sx={{ mr: -1, color: colors.gray7, '&:hover': { color: colors.white, bgcolor: colors.gray7 } }}><Close fontSize="small" /></IconButton></Tooltip> : undefined }} sx={{ width: { xs: '100%', sm: 320 }, bgcolor: colors.white, '& .MuiInputBase-input': { fontSize: 14 }, '& .MuiInputBase-input::placeholder': { fontSize: 14, opacity: 1 } }} />
                </Box>
                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 1.25 }}>
                  <Box sx={{ border: `1px solid ${colors.managementBorder}`, borderRadius: 1, overflow: 'hidden' }}>
                    ...
                    <Stack spacing={0.5} sx={{ p: 0.75, maxHeight: 250, overflowY: 'auto' }}>
                      {matchingPositions.filter((position) => !selectedPositions.includes(position)).map((position) => <Button key={position} variant="text" color="primary" endIcon={<MoveIcon direction="right" />} onClick={() => addPosition(position)} disabled={!canEdit} sx={{ justifyContent: 'space-between', textAlign: 'left', minHeight: 34, color: colors.blue4, fontSize: 12.5, '&:hover .move-outline': { display: 'none' }, '&:hover .move-filled': { display: 'block' } }}>{position.toLocaleUpperCase('es-CL')}</Button>)}
                  ...
                  <Box sx={{ border: `1px solid ${colors.managementBorder}`, borderRadius: 1, overflow: 'hidden', bgcolor: colors.white }}>
                    ...
                    <Stack spacing={0.5} sx={{ p: 0.75, maxHeight: 250, overflowY: 'auto' }}>
                      {selectedPositions.filter((position) => matchingPositions.includes(position)).map((position) => <Button key={position} variant="text" color="primary" startIcon={<MoveIcon direction="left" />} onClick={() => removePosition(position)} disabled={!canEdit} sx={{ justifyContent: 'flex-start', textAlign: 'left', minHeight: 34, fontWeight: 800, color: colors.blue4, fontSize: 12.5, '&:hover .move-outline': { display: 'none' }, '&:hover .move-filled': { display: 'block' } }}>{position.toLocaleUpperCase('es-CL')}</Button>)}
              ...
              <Stack spacing={1.5}>
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1, flexWrap: 'wrap' }}>
                  ...
                  <Stack direction="row" spacing={0.75}>
                    <ToggleButton value="email" selected={channels.includes('email')} onChange={() => toggleChannel('email')} disabled={!canEdit} sx={channelToggleSx}>{channels.includes('email') ? <EmailOutlined sx={{ fontSize: 17 }} /> : <MailX size={17} />}Correo</ToggleButton>
                    <ToggleButton value="whatsapp" selected={channels.includes('whatsapp')} onChange={() => toggleChannel('whatsapp')} disabled={!canEdit} sx={channelToggleSx}>{channels.includes('whatsapp') ? <WhatsApp sx={{ fontSize: 17 }} /> : <MessageCircleOff size={17} />}WhatsApp</ToggleButton>
                  </Stack>
                </Box>
                <input ref={fileInputRef} type="file" accept="application/pdf,.pdf" hidden onChange={(event) => {
                  const file = event.target.files?.[0] || null
                  if (file && (file.type !== 'application/pdf' || file.size > 10 * 1024 * 1024)) { setToast({ type: 'error', text: 'Selecciona un PDF de hasta 10 MB.' }); event.target.value = ''; return }
                  setAttachment(file)
                }} />
                <input value={title} onChange={(event) => setTitle(event.target.value)} disabled={!canEdit} placeholder="Asunto" aria-label="Asunto" style={{ width: '100%', minHeight: 44, border: `1px solid ${colors.managementBorder}`, borderRadius: 6, padding: '10px 12px', font: 'inherit', boxSizing: 'border-box' }} />
                <textarea value={message} onChange={(event) => setMessage(event.target.value)} disabled={!canEdit} placeholder="Escribe el mensaje" aria-label="Mensaje" rows={5} style={{ width: '100%', border: `1px solid ${colors.managementBorder}`, borderRadius: 6, padding: 12, font: 'inherit', resize: 'vertical', boxSizing: 'border-box' }} />
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1, flexWrap: 'wrap' }}>
                  <Box>{attachment ? <Chip icon={<PictureAsPdfOutlined />} label={`${attachment.name} · ${(attachment.size / 1024 / 1024).toFixed(1)} MB`} onDelete={canEdit ? () => setAttachment(null) : undefined} /> : <Button startIcon={<AttachFile />} onClick={() => fileInputRef.current?.click()} disabled={!canEdit}>Adjuntar PDF</Button>}</Box>
                  <Button variant="contained" size="large" startIcon={<SendOutlined />} onClick={() => void send()} disabled={!canEdit || sending}>{sending ? 'Enviando...' : 'Crear y enviar'}</Button>
                </Box>
              </Stack>
            </Paper>
            ...
            <Paper variant="outlined" sx={{ borderColor: colors.managementBorder }}>
              ...
              {campaigns.length === 0 ? <Typography sx={{ p: 2, color: colors.gray4 }}>Aún no existen campañas en este proyecto.</Typography> : <List disablePadding>{campaigns.map((campaign, index) => <ListItem key={campaign.id} divider={index < campaigns.length - 1} secondaryAction={<Button size="small" onClick={() => void openCampaign(campaign.id)} disabled={detailLoading}>Ver</Button>}><ListItemText primary={<Typography sx={{ fontWeight: 700 }}>{campaign.title}</Typography>} secondary={`${formatDateTime(campaign.created_at)} · ${(campaign.channels || []).join(' + ')}${campaign.attachment_name ? ` · ${campaign.attachment_name}` : ''}`} /></ListItem>)}</List>}
            </Paper>
          </>}
        </Stack>
      </Box>
